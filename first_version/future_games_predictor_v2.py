...
class FutureGamesPredictor:
    """Predictor optimizado para futuros juegos basado en ultimas 3 partidas"""
    ...
    def load_data(self):
        """Carga datos y obtiene ultimas 3 partidas"""
        ...
        # Obtener ultimas 3 partidas completadas
    ...
    def analyze_last_3_patterns(self):
        """Analiza patrones especificos de las ultimas 3 partidas"""
        patterns = {
            'mine_positions': [],
        ...
        print(f"Analizando ultimas 3 partidas: {self.last_3_games}")
    ...
    def calculate_adaptive_probabilities(self, patterns):
        """Calcula probabilidades adaptandose a las ultimas 3 partidas"""
        prob_matrix = np.ones((5, 5)) * 0.1  # Base minima
        
        # Factor 1: Frecuencia directa en ultimas 3 partidas (40% peso)
        ...
        # Factor 3: Adaptacion geometrica (30% peso)
        ...
        # Factor 4: Anti-repeticion (reducir posiciones muy recientes)
        for (col, row), count in patterns['position_count'].items():
            if count >= 2:  # Aparecio 2+ veces
                prob_matrix[row-1, col-1] *= 0.7  # Reducir 30%
        
        # Normalizar para ~3 minas totales
        prob_matrix = prob_matrix / np.sum(prob_matrix) * 3.0
        prob_matrix = np.maximum(prob_matrix, 0.005)  # Minimo 0.5%
        
        return prob_matrix
    
    def apply_geometry_adaptation(self, prob_matrix, geometry_type):
        """Aplica adaptacion segun tipo de geometria dominante"""
    ...
    def get_top_predictions(self, prob_matrix, patterns):
        """Obtiene las top 3 predicciones con justificacion"""
        ...
        for i, idx in enumerate(top_indices):
            ...
            if (col, row) in patterns['position_count']:
                ...
                justification.append(f"Aparecio {count} veces en ultimas 3 partidas")
            ...
            if row in [1, 5] or col in [1, 5]:
                justification.append("Posicion de borde")
            
            predictions.append({
                'position': (col, row),
                'probability': prob,
                'justification': " + ".join(justification) if justification else "Patron estadistico adaptativo"
            })
    ...
    def get_safe_zones(self, prob_matrix):
        """Identifica zonas mas seguras con analisis detallado"""
        safe_positions = []
    ...
    def analyze_strategic_zones(self, prob_matrix):
        """Analiza zonas estrategicas para diferentes estilos de juego"""
        strategies = {
            'conservador': [],
            'balanceado': [],
            'agresivo': []
    ...
    def predict_next_game(self):
        """Predice la siguiente partida basandose en las ultimas 3"""
        print("PREDICTOR PARA FUTUROS JUEGOS - BASADO EN ULTIMAS 3 PARTIDAS")
        print("=" * 70)
        
        print(f"\nPREDICCION PARA PARTIDA {self.next_game_number}")
        
        # Analizar patrones de las ultimas 3 partidas
        patterns = self.analyze_last_3_patterns()
        
        # Mostrar resumen de patrones
        print(f"\nRESUMEN DE PATRONES (ultimas 3 partidas):")
        print(f"- Total minas analizadas: {len(patterns['mine_positions'])}")
        print(f"- Posiciones mas frecuentes: {dict(sorted(patterns['position_count'].items(), key=lambda x: x[1], reverse=True)[:3])}")
        print(f"- Filas mas activas: {dict(sorted(patterns['row_count'].items(), key=lambda x: x[1], reverse=True)[:3])}")
        print(f"- Columnas mas activas: {dict(sorted(patterns['col_count'].items(), key=lambda x: x[1], reverse=True)[:3])}")
        print(f"- Tipos de geometria: {patterns['geometry_types']}")
        ...
        # Analisis por filas y columnas
        safe_rows, safe_cols = self.analyze_safe_rows_cols(prob_matrix)
        
        print(f"\n=== ANALISIS POR FILAS ===")
        print("Filas ordenadas por seguridad (menor riesgo = mas segura):")
        for i, row_info in enumerate(safe_rows[:3], 1):
            print(f"{i}. FILA {row_info['row']}: {row_info['avg_risk']:.1f}% riesgo promedio")
            print(f"   -> Casilla mas segura: ({row_info['safest_cell']},{row_info['row']}) con {row_info['safest_risk']:.1f}% riesgo")
            print(f"   -> Casillas bajo 5% riesgo: {row_info['cells_under_5pct']}/5")
        
        print(f"\n=== ANALISIS POR COLUMNAS ===")
        print("Columnas ordenadas por seguridad (menor riesgo = mas segura):")
        for i, col_info in enumerate(safe_cols[:3], 1):
            print(f"{i}. COLUMNA {col_info['col']}: {col_info['avg_risk']:.1f}% riesgo promedio")
            print(f"   -> Casilla mas segura: ({col_info['col']},{col_info['safest_cell']}) con {col_info['safest_risk']:.1f}% riesgo")
        ...
        if strategies['conservador']:
            ...
            print(f"   No hay casillas ultra-seguras disponibles")
        
        print(f"\nBALANCEADO (maximo 6% riesgo):")
        for i, move in enumerate(strategies['balanceado'][:6], 1):
            print(f"   {i}. ({move['pos'][0]},{move['pos'][1]}) - {move['risk']:.1f}% riesgo")
        
        print(f"\nAGRESIVO (maximo 10% riesgo):")
        ...
        for pred in predictions:
            pos = pred['position']
            prob = pred['probability']
            print(f"[X] ({pos[0]},{pos[1]}) - {prob:.1f}% probabilidad de mina")
        
        # Analisis adicional de tendencias
        print(f"\n=== ANALISIS DE TENDENCIAS ===")
        self.analyze_advanced_patterns(patterns, prob_matrix)
        
        print(f"\n{'='*70}")
        print(f"RESUMEN: {len(safe_zones)} casillas seguras detectadas")
        print(f"RECOMENDACION: Comenzar con la fila {safe_rows[0]['row']} o columna {safe_cols[0]['col']}")
        print(f"SIGUIENTE ACTUALIZACION: Despues de completar partida {self.next_game_number}")
        print(f"{'='*70}")
        
        return predictions, safe_zones
    
    def analyze_advanced_patterns(self, patterns, prob_matrix):
        """Analisis avanzado de patrones y tendencias"""
        
        # Analisis de distribucion temporal
        print(f"DISTRIBUCION TEMPORAL:")
        recent_positions = patterns['mine_positions'][-6:]  # Ultimas 6 minas
        ...
        print(f"   - Cambio en dispersion: {self.analyze_dispersion_change(patterns)}")
        
        # Analisis de zonas evitadas
        avoided_zones = self.find_avoided_zones(prob_matrix)
        if avoided_zones:
            print(f"ZONAS SISTEMATICAMENTE EVITADAS:")
    ...
    def analyze_dispersion_change(self, patterns):
        """Analiza cambio en la dispersion de las minas"""
        ...
        if recent_avg > early_avg + 0.5:
            return "Incrementando dispersion"
        elif recent_avg < early_avg - 0.5:
            return "Reduciendo dispersion"
        else:
            return "Dispersion estable"
    
    def find_avoided_zones(self, prob_matrix):
        """Identifica zonas sistematicamente evitadas"""
    ...
    def analyze_correlations(self, patterns):
        ...
        if repeated_positions:
            correlations.append(f"Posiciones con repeticion detectada: {repeated_positions}")
...
def main():
    """Funcion principal para uso futuro"""
