...
def custom_mines_loss(y_true, y_pred):
    """Función de pérdida personalizada: BCE + regularización para M=4 minas"""
    # Binary Cross Entropy para cada celda
    bce = tf.keras.losses.binary_crossentropy(y_true, y_pred)
    bce_loss = tf.reduce_mean(bce)
    
    # Regularización: penalizar si la suma de probabilidades no está cerca de 4
    prob_sum = tf.reduce_sum(y_pred, axis=1)  # Suma de probabilidades por muestra
    target_sum = 4.0  # Esperamos exactamente 4 minas
    sum_penalty = tf.reduce_mean(tf.square(prob_sum - target_sum))
    
    # Pérdida total
    total_loss = bce_loss + 0.1 * sum_penalty
    return total_loss

def precision_at_4(y_true, y_pred):
    """Precision@4: proporción de minas reales en el top-4 predicho"""
    # Obtener top-4 predicciones
    top_4_indices = tf.nn.top_k(y_pred, k=4).indices
    
    # Crear máscara binaria para top-4
    batch_size = tf.shape(y_pred)[0]
    indices = tf.stack([
        tf.repeat(tf.range(batch_size), 4),
        tf.reshape(top_4_indices, [-1])
    ], axis=1)
    
    top_4_binary = tf.scatter_nd(
        indices, 
    ...
    intersection = tf.reduce_sum(y_true * top_4_binary, axis=1)
    precision = tf.reduce_mean(intersection / 4.0)
    return precision

def recall_at_4(y_true, y_pred):
    """Recall@4: proporción de minas reales encontradas en top-4"""
    # Obtener top-4 predicciones
    top_4_indices = tf.nn.top_k(y_pred, k=4).indices
...
class MinesTransformer:
    ...
    def build_model(self) -> Model:
        ...
        x = layers.Dense(256, activation="relu", name="classification_head_1")(combined_features)
        x = layers.Dropout(self.dropout_rate)(x)
        x = layers.Dense(128, activation="relu", name="classification_head_2")(x)
        x = layers.Dropout(self.dropout_rate)(x)
...
def create_default_model(sequence_length: int, feature_dim: int) -> MinesTransformer:
    """Crea modelo con configuración por defecto optimizada"""
    
    model = MinesTransformer(
        sequence_length=sequence_length,
        feature_dim=feature_dim,
        embed_dim=32,  # Reducido para dataset pequeño
        num_heads=4,
        ff_dim=64,  # Reducido para evitar overfitting
        num_transformer_blocks=3,  # Reducido
        dropout_rate=0.2,  # Aumentado para regularización
        output_dim=25
    )
...
if __name__ == "__main__":
    ...
    model = create_default_model(sequence_length=75, feature_dim=15)  # 3 juegos × 25 celdas
